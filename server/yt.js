...
function getvideodata(req,res){
    ...
    fetch("https://ytdl.socialplug.io/api/video-info?url="+url,{
        ...
        res.json({formats:data.format_options.video.mp4,title:data.title,thumbnail:data.image})
    })
...
function createjob(req,res){
    ...
    let qualities = ["144p", "240p", "360p", "480p", "720p", "1080p", "1440p", "2160p"]
    ...
    url = url.replaceAll("/","%2F")
    fetch("https://ytdl.socialplug.io/api/start-download?url="+url+"&quality="+quality,{
        headers:{
            "if-none-match":'W/"b9-p25sIP90/JNqBygRCpU2i6EwumA"',
            "origin":"https://www.socialplug.io"
        }
